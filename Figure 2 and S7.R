...
# metadata table
metadata <- read.csv("metadata_16S_W5.csv", header = 1, row.names = 1, sep = ",")

###beta-diversity####
###all samples (bulk and rhizosphere)
all.dist <- vegdist(t(com), method = "bray", binary = F)
all.pcoa <- cmdscale(all.dist, k=3, eig = T)
all_pcoa_points <- as.data.frame(all.pcoa$points)
sum_eig <- sum(all.pcoa$eig)
eig_percent <- round(all.pcoa$eig/sum_eig*100, 1)
...
set.seed(1)
all.div <- adonis2(all.dist ~ Soil_type, data = all_pcoa_result, permutations = 999, method = "bray")
all.div #R2=0.0802,p=0.001 ***

#all.div <- adonis2(all.dist ~ Variety, data = all_pcoa_result, permutations = 999, method = "bray")
#all.div # R2=0.12251, p=0.001 ***

#all.div <- adonis2(all.dist ~ Treatment, data = all_pcoa_result, permutations = 999, method = "bray")
#all.div # R2=0.13068, p=0.001 ***

## legend order
all_pcoa_result$Variety <- factor(all_pcoa_result$Variety,
                                  levels=c("Bulk soil", "ATOL", "DESIREE", "JELLY",  "KRAB", "PASJA_POMORSKA", "RUDAWA", "SALTO"),
                                  labels = c("Bulk soil", "ATOL", "DESIREE", "JELLY", "KRAB", "PASJA POMORSKA", "RUDAWA", "SALTO"))

Bac3 <- ggplot(all_pcoa_result, aes(x=PCoA1, y=PCoA2, color=Variety, group=Variety,fill =Variety,shape = Soil_type)) +
  labs(x=paste("PCoA 1 (", eig_percent[1], "%)", sep = ""), 
       y=paste("PCoA 2 (", eig_percent[2], "%)", sep = "")) +
  geom_point(size=4, alpha=1.0) +
  #geom_encircle(aes(fill=Treatment),alpha=0.4, show.legend = T) +
  ...
  scale_color_manual(values=c("#a3b07d","#7799bc","#f6e0a5","#c98bb5","#a4757d","#e5977d","#8580a3","#535969")) +
  scale_fill_manual(values=c("#a3b07d","#7799bc","#f6e0a5","#c98bb5","#a4757d","#e5977d","#8580a3","#535969"))+
  mytheme +
  stat_ellipse(aes(group = Variety, color = Variety, fill = Variety), level = 0.8, geom = "polygon", alpha = 0.1)
...
###Fungi_import data####
# feature table
com <- read.csv("otu_table_ITS_W5.csv", header = 1, row.names = 1, sep = ";") 

# metadata table
metadata <- read.csv("metadata_ITS_W5.csv", header = 1, row.names = 1, sep = ",")
...
all_pcoa_result$Variety <- factor(all_pcoa_result$Variety,
                                 levels=c("bulk", "ATOL", "DESIREE", "JELLY",  "KRAB", "PASJA_POMORSKA", "RUDAWA", "SALTO"),
                                 labels = c("Bulk soil", "ATOL", "DESIREE", "JELLY", "KRAB", "PASJA POMORSKA", "RUDAWA", "SALTO"))

Fun3 <- ggplot(all_pcoa_result, aes(x=PCoA1, y=PCoA2, color=Variety, group=Variety,fill =Variety,shape = Soil_type)) +
  labs(x=paste("PCoA 1 (", eig_percent[1], "%)", sep = ""), 
       y=paste("PCoA 2 (", eig_percent[2], "%)", sep = "")) +
  geom_point(size=4, alpha=1.0) +
  #geom_encircle(aes(fill=Treatment),alpha=0.4, show.legend = T) +
...
### adonis statistic####
set.seed(1)
all.div <- adonis2(all.dist ~ Soil_type, data = sample_info, permutations = 999, method = "bray")
all.div # R2=0.16551,p=0.001 ***

## merge figures
FigS7 <- Bac3 + Fun3 + plot_layout(nrow = 1, guides= "collect")&
  theme(legend.position = 'right')
